    ...
    for i in range(df.shape[0]):
        ...
        try:
            query = db.insert(Songs).values(song_ID=i, track_ID=df.iloc[i][1], artists=df.iloc[i][2],
                                            album_name=df.iloc[i][3], track_name=df.iloc[i][4], popularity=int(
                                                df.iloc[i][5]),
                                            ...
                                            liveness=df.iloc[i][16], valence=df.iloc[i][17], tempo=df.iloc[i][18], time_signature=float(
                df.iloc[i][19]),
...
def Initialize_Playlists():
    ...
    Playlists = db.Table('Playlists', metadata,
                         db.Column('playlists_ID', db.Integer(),
                                   primary_key=True, nullable=False),
                         db.Column('playlist_name', db.String(
                             255), nullable=False),
